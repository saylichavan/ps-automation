...
const logouttext = locator.logouttext;

// Define fake input data
const fakeUserNameKeyword = fake.username;
const fakePasswordKeyword = fake.password;

let usernameInput, passwordInput, loginButton, resultStat, loggedoutButton;
...
Page.prototype.logout = async function () {
    ...
    loggedoutButton = await this.findlogoutButton();
    await loggedoutButton.click();

    const pageSource = await this.driver.getPageSource();

    return await this.driver.wait(async function () {
        return pageSource.includes('Login');
    }, 5000);
};
